...
import {
  ...
  AlertCircle, CheckCircle2, Clock, ChevronRight, X, Receipt, Fingerprint
} from 'lucide-react';
  ...
  return (
    <div className="app-container" style={{ background: '#f8fafc', display: 'flex' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: '100vh', overflow: 'hidden' }}>
        ...
        {/* ── HEADER ── */}
        <header style={{ height: '70px', padding: '0 3rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(226,232,240,0.8)', position: 'sticky', top: 0, zIndex: 100 }}>

          {/* Search */}
          <div style={{ display: 'flex', alignItems: 'center', background: '#f1f5f9', borderRadius: '10px', padding: '8px 16px', width: '300px', gap: '10px' }}>
            <Search size={18} color="#64748b" />
            <input type="text" placeholder="Tìm kiếm nhanh..." style={{ border: 'none', background: 'transparent', outline: 'none', fontSize: '0.9rem', width: '100%' }} />
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
            ...
            <div ref={notifRef} style={{ position: 'relative' }}>
              <button
                ...
                style={{ position: 'relative', padding: '10px', borderRadius: '12px', border: '1px solid #e2e8f0', background: showNotifPanel ? '#eff6ff' : 'white', color: showNotifPanel ? '#3b82f6' : '#64748b', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}>
                <Bell size={20} />
                {unreadCount > 0 && (
                  <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} style={{ position: 'absolute', top: 6, right: 6, minWidth: 16, height: 16, background: '#ef4444', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.65rem', fontWeight: '700', color: 'white', padding: '0 3px', lineHeight: 1 }}>
                    {unreadCount}
                  </motion.span>
                )}
              </button>
            ...
            <div style={{ position: 'relative' }}>
              <button onClick={() => setShowProfileMenu(!showProfileMenu)}
                style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '6px 6px 6px 14px', borderRadius: '14px', border: '1px solid #e2e8f0', background: 'white', cursor: 'pointer', transition: 'all 0.2s', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          ...
          <motion.main key={role + location.pathname} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} style={{ padding: '1rem 3rem 3rem' }}>
            {children}
          </motion.main>
        </div>
      </div>
